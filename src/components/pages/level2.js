...
// Level 2 specific words - all 3 letters, slightly more challenging than level 1
const level2Words = [
  "ace", "aim", "bay", "dry", "ego", 
  "fin", "gym", "hex", "ink", "jaw",
  "key", "lab", "mix", "new", "oak",
...
function Level2Game() {
  ...
  useEffect(() => {
    const successRate = (score / (level2Words.length * 10)) * 100;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (currentIndex >= level2Words.length) {
      ...
      const newWord = level2Words[currentIndex];
    ...
    const moveObstacles = setInterval(() => {
      setObstacleList((prevObstacles) => {
        const updatedObstacles = prevObstacles.map((obstacle) => ({
          ...
          position: {
            ...obstacle.position,
            left: `${parseFloat(obstacle.position.left) - 1.2}%`,
          },
        }));
  ...
  const handleWordMatch = () => {
    ...
    if (wordsCompleted + 1 === level2Words.length) {
      setAllWordsCompleted(true);
    }
  };
        ...
        {gameOver && (
          <div style={{
            ...
            {levelPassed && (
              <>
                <button
                  onClick={() => window.location.href = "/l3"}
                  style={{
                    padding: "10px 20px",
                    margin: "10px",
